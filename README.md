...
rm -rf $HOME/.config/ghostty; rm -rf $HOME/.bash*; rm -rf $HOME/.zsh*; rm -rf $HOME/.oh-my-zsh; rm -rf $HOME/.tmux*; rm -rf $HOME/.config/nvim; rm -rf $HOME/.vim*; rm -rf $HOME/.emacs*; stow -d $HOME/.stowed stow ghostty tmux fzf bat delta starship bash zsh nvim vim emacs
```
...
### 1) Mise - A development environment manager

```bash
# Install Mise-En-Place
curl https://mise.run | sh

# Ensure mise is loaded with the shell
echo 'eval "$(~/.local/bin/mise activate zsh)"' >> ~/.zshrc
```

### 2) Nix Package Manager - A package manager with a massive amount of open-source installation images
...
### 4) Worktrunk - A Git worktree utility

```bash
# Rust cargo dependency
nix-env -iA nixpkgs.cargo

# Worktrunk installation
cargo install worktrunk && wt config shell install
```

### 5) Gitui - Terminal Git TUI (Rust based)
...
### 6) Lazygit - Another Terminal Git TUI
